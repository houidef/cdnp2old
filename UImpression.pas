...
type
  ...
  public
    ...
    function GetTailleText(a:TCanvas; b:AnsiString):dword;//00519AA8
    function GetTailleColonne(ImpDateNais:boolean; ImpRedoublage:boolean):dword;//00519AF8 get la taille de 1ere colone
    function NbrLigneBasTableau:dword;//00519E00
    function GetTextHeightOfTableau:dword;//00519E58
...
begin//0
  ...
  MargeWidth := MargeRight - MargeLeft - Printer.PageWidth;
  MargeHeight := MargeButtom - Marge_Top - Printer.PageHeight;
  Fwidth := Printer.PageWidth;
  FHeight := Printer.PageHeight;
  if (MargeHeight < 100) then
  begin//00519A43
    MargeHeight := 100;
  end;//1
  if (Marge_Top < 100) then
  begin//00519A50
    Marge_Top := 100;
  end;//1
  if (MargeLeft < 100) then
  begin//1
    //00519A5D
    MargeLeft := 100;
  end;//1
  if (MargeWidth < 100) then
  begin//1
    //00519A6A
    MargeWidth := 100;
  end;//1
...
//00519AA8
function TImpression.GetTailleText(a:TCanvas; b:AnsiString):dword; //get taille text
begin//0
    result := a.TextWidth(b);
end;//0

//00519AF8
function TImpression.GetTailleColonne(ImpDateNais:boolean; ImpRedoublage:boolean):dword; //get la taille de 1ere colone
var
 I,Nbr : integer;
 buf,EleveName,text,lvar_130 : string;
begin
  //00519AF8
  result :=0;
    //00519B40
    ...
    if (FInclureImpression <> nil) then//00519CAB
        ...
        begin//00519CCB
          if (FInclureImpression.FC[I] = false) then
		  begin
			  lvar_130 :=  FInclureImpression.f8[I] + ' ';
			  if (GetTailleText(FCanvas, lvar_130) > result) then 
			  result := GetTailleText(FCanvas, lvar_130);
		  end;
        end;//4
    if (FImprNbrEleve) then //00519D3C
      result := GetTailleText(FCanvas, IntToStr(Nbr)) + result + 10;
    result := result + 20;//EBX
end;//0
...
//00519E58
function TImpression.GetTextHeightOfTableau:dword;
begin//0
...
begin//0
  result := ((FCanvas.TextHeight('ALEXANDRE') + 4) * NbrLigneBasTableau + GetTextHeightOfTableau + 140 < Printer.PageHeight);
end;//0
